<?php
	...
	if(isset($_POST['submenu_editlayout_submit']))  //check if "mobile_nav_editlayout_submit" submit button is clicked
	{
		...
		if($_POST["submenu_layout_template"]=="1")
		{
			$mobile_navname="Basic_".str_replace(" ","_",$_POST["submenu_layout_name"]);  //submenu layout number to save 
		}
			...
			<div class="row editsection">
				<div class="col-md-12">
					<select name="editsavedlayout" id="editsavedlayout">
						...
						?>
							<option value="<?php echo str_replace("submenu_layout_","",$layout); ?>" <?php if(get_option('udm_submenu_default')==str_replace("submenu_layout_","",$layout)){ echo ' class="selectedlayout"'; } ?>><?php echo str_replace("_"," ", str_replace("submenu_layout_","",$layout)); ?></option>
						<?php	
